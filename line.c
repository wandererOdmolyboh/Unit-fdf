...
/*   Updated: 2019/02/08 06:21:16 by dmolyboh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void line(void *mlx_ptr, void *win_ptr, double x1, double y1, double x2, double y2)
{
	///структуры!
    double dx;
    double dy;
	...
	if (lengthY <= lengthX)
    {
        ...
        while(length--)
        {
             mlx_pixel_put(mlx_ptr, win_ptr,  x + c,   y + d1, 0x00FFFF);
            x += dx;
            d += 2 * lengthY;
            if (d > 0)
            {
